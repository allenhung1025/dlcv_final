...
from .dataset import CUB, CarsDataset, NABirds, dogs, INat2017, food, FoodMultiModal
...
def get_loader(args):
    ...
    if args.dataset == "food":
        ...
        test_transform=[transforms.Resize((600, 600), Image.BILINEAR),
                                    ...
                                    transforms.ToTensor(),
                                    transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])]
        trainset = FoodMultiModal(args.data_root, args.emb_file, "../food_data/label2name.txt", train_transform, mode="train", image_type="all") 
        testset = FoodMultiModal(args.data_root, args.emb_file, "../food_data/label2name.txt", test_transform, mode="val", image_type="all") 
        #testsets = {image_type: FoodMultiModal(args.data_root, args.emb_file, "../food_data/label2name.txt", test_transform, mode="val", image_type=image_type) for image_type in ['all', 'f', 'c', 'r'] }
...
def get_val_loader(data_root):
    ...
    test_loaders = []
    for _type in ['all', 'f', 'c', 'r']:

        dataset = FoodMultiModal(data_root, None, "../food_data/label2name.txt", test_transform, mode="val", image_type=_type)
        ...
        test_loader = DataLoader(dataset,
                             ...
                             batch_size=1,
                             num_workers=4,
                             pin_memory=True) if dataset is not None else None
