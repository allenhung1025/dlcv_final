...
class FoodMultiModal(Dataset):
    def __init__(self, root, embd_file, label2name, transforms_=None, mode="train", image_type="c"):
        ...
        self.image_files = [] 
        self.embd_file = embd_file
        if embd_file is not None:
            self.embd_dict  = pickle.load(open(embd_file, 'rb'))
    ...
    def __getitem__(self, idx):
        ...
        if self.embd_file is not None:
            embd = torch.tensor(self.embd_dict[chinese_label]).squeeze(0)
            return {'img':img, 'label':label, 'chinese_label':chinese_label, 'embd': embd}
        else:
