...
class AuxCLF(nn.Module):
    ...
    def forward(self, x):
        return self.clf(x).flatten() if self.activate is not None else self.clf(x)

class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, loss_type='ce', smoothing_value=0, aux_type=None, aux_loc=None, zero_head=False):
        super(VisionTransformer, self).__init__()
        self.num_classes      = num_classes
        self.loss_fct         = get_loss_func(loss_type, smoothing_value=smoothing_value, num_classes=num_classes)
        self.zero_head        = zero_head
        self.classifier       = config.classifier
        self.transformer      = Transformer(config, img_size)
        self.part_head        = Linear(config.hidden_size, num_classes)
        self.aux_loc          = aux_loc
        if aux_type == 'category':
            self.aux_clf      = AuxCLF(num_classes if aux_loc == 'serial' else config.hidden_size, 3)
            self.aux_loss_fct = get_loss_func(loss_type, smoothing_value=smoothing_value, num_classes=3)
        elif aux_type == 'number':
            self.aux_clf      = AuxCLF(num_classes if aux_loc == 'serial' else config.hidden_size, 1, activate=True)
            self.aux_loss_fct = nn.MSELoss()
        else:
            self.aux_clf      = None
            self.aux_loss_fct = None
        
        
    def forward(self, x, labels=None, freq=None):
        part_tokens = self.transformer(x)
        part_logits = self.part_head(part_tokens[:, 0])
        
        if labels is not None:
            part_loss     = self.loss_fct(part_logits.view(-1, self.num_classes), labels.view(-1))
            contrast_loss = con_loss(part_tokens[:, 0], labels.view(-1))
            loss          = part_loss + contrast_loss
            if freq is not None and self.aux_clf is not None:
                cat_logits = self.aux_clf(part_logits if self.aux_loc == 'serial' else part_tokens[:, 0])
                aux_loss   = self.aux_loss_fct(cat_logits, freq)
                loss      += aux_loss
