...
def freqcat2class(cat):
    return ['r', 'c', 'f'].index(cat)
...
class food(Dataset):
    def __init__(self, root, label2name, transforms_=None, mode="train", image_type="c"):
        ...
        if image_type != "all":
            ...
            image_frqunecies_cat    = [['r', 'c', 'f'].index(cat) for cat in retrieved_label2name_df[1].tolist()]
            ...
            image_frqunecies_cat  = [['r', 'c', 'f'].index(cat) for cat in label2name_df[1].tolist()]

        directory = os.path.join(root, mode)
        for image_cls, image_cls_chinese, image_frqunecy_cat in zip(image_classes, image_classes_chinese, image_frqunecies_cat):
            image_cls_dir = os.path.join(directory, str(image_cls))
            for image in os.listdir(image_cls_dir):
                image_frequecy = len(os.listdir(image_cls_dir)) 
                image_file     = os.path.join(image_cls_dir, image)
                self.image_files.append([image_file, int(image_cls), image_cls_chinese, image_frequecy, image_frqunecy_cat])
    ...
    def __getitem__(self, idx):
        ...
        freq          = self.image_files[idx][3]
        freq_cat      = self.image_files[idx][4]
        return {'img':img, 'label':label, 'chinese_label': chinese_label, 'freq': freq, 'freq_cat': freq_cat}
